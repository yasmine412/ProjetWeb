...
class LogementRepository extends ServiceEntityRepository
{
    ...
    public function findBySearchBarFiltres($location,$datedebut,$datefin,$nb_voyageurs,$prix,$type_logement,$chambres,$lits,$salledeau,$type_propriete) {
        if ($location == " ") {
            $location = "";
        }
        if ($datedebut == " ") {
       ...
       AND nbr_sdb >= ?';
        ...
        $stmt = $conn->prepare($sql);
        if (($type_logement == 7)&&($type_propriete == 7)) {
            $stmt->execute(['%'.$location.'%',$nb_voyageurs,$prix, $chambres, $lits, $salledeau]);
        }
        else if ($type_logement == 7) {
            $stmt->execute(['%'.$location.'%',$nb_voyageurs,$prix, $chambres, $lits, $salledeau, $type_propriete]);
        }
        else if ($type_propriete == 7) {
            $stmt->execute(['%'.$location.'%',$nb_voyageurs,$prix, $chambres, $lits, $salledeau, $type_logement]);
        }
        else {
            $stmt->execute(['%'.$location.'%',$nb_voyageurs,$prix, $chambres, $lits, $salledeau, $type_propriete, $type_logement]);
        }

        return $stmt->fetchAll(PDO::FETCH_OBJ);
    }

    public function findByFiltreAndCarousel($prix,$type_logement,$chambres,$lits,$salledeau,$type_propriete,$filtrec) {
        if ($type_logement == " ") {
            $type_logement = "";
        }
        if ($chambres=="Tout") {
       ...
       AND description LIKE ?';
        ...
        $stmt = $conn->prepare($sql);
        if (($type_logement == 7)&&($type_propriete == 7)) {
            $stmt->execute([$prix, $chambres, $lits, $salledeau, '%'.$filtrec.'%']);
        }
        else if ($type_logement == 7) {
            $stmt->execute([$prix, $chambres, $lits, $salledeau,'%'.$filtrec.'%', $type_propriete]);
        }
        else if ($type_propriete == 7) {
            $stmt->execute([$prix, $chambres, $lits, $salledeau,'%'.$filtrec.'%', $type_logement]);
        }
        else {
            $stmt->execute([$prix, $chambres, $lits, $salledeau, $type_propriete,'%'.$filtrec.'%', $type_logement]);
        }
        return $stmt->fetchAll(PDO::FETCH_OBJ);

    }

    public function findBySearchBarFiltresCarousel($location,$datedebut,$datefin,$nb_voyageurs,$prix,$type_logement,$chambres,$lits,$salledeau,$type_propriete,$filtrec) {
        if ($location == " ") {
            $location = "";
        }
        if ($datedebut == " ") {
       ...
       AND description LIKE ?';
        ...
        $stmt = $conn->prepare($sql);
        if (($type_logement == 7)&&($type_propriete == 7)) {
            $stmt->execute(['%'.$location.'%',$nb_voyageurs,$prix, $chambres, $lits, $salledeau,'%'.$filtrec.'%']);
        }
        else if ($type_logement == 7) {
            $stmt->execute(['%'.$location.'%',$nb_voyageurs,$prix, $chambres, $lits, $salledeau,'%'.$filtrec.'%', $type_propriete]);
        }
        else if ($type_propriete == 7) {
            $stmt->execute(['%'.$location.'%',$nb_voyageurs,$prix, $chambres, $lits, $salledeau,'%'.$filtrec.'%', $type_logement]);
        }
        else {
            $stmt->execute(['%'.$location.'%',$nb_voyageurs,$prix, $chambres, $lits, $salledeau,'%'.$filtrec.'%', $type_propriete, $type_logement]);
        }
